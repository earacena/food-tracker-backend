...
import { zMealIdParams } from '../mealEntries/mealEntry.types'
...
export async function getActivityByIdController (
  ...
  try {
    ...
    const activity = zActivity.parse(
...
export async function getActivitiesByUserIdController (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    const userActivities = zActivities.parse(
      await ActivityRepository.findActivitiesByUserId(userId)
    )
...
export async function createActivityController (
  ...
  try {
    ...
    const newActivity = zActivity.parse(
...
export async function deleteActivitiesByMealIdController (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  try {
    ...
    await ActivityRepository.deleteActivitiesByMealId(mealId)
